...
void log_interpreter::interp_init()
{
...
void log_interpreter::print_nc_line_number()
{
    char text[256];
    int k;
    int m;
...
void log_interpreter::offset_origin(const Position& pos)
{
...
void log_interpreter::units(Units u)
{
    if (u == Units::Imperial)
    {
...
void log_interpreter::rapid_rate(double rate)
{
...
void log_interpreter::rapid(const Position& pos)
{
...
void log_interpreter::feed_rate(double rate)
{
...
void log_interpreter::feed_reference(FeedReference reference)
{
    PRINT("SET_FEED_REFERENCE(%s)\n", (reference == FeedReference::Workpiece) ? "CANON_WORKPIECE" : "CANON_XYZ");
}


void log_interpreter::motion_mode(Motion mode)
{
...
void log_interpreter::plane(Plane pl)
{
...
void log_interpreter::cutter_radius_comp(double radius)
{
	PRINT("SET_CUTTER_RADIUS_COMPENSATION(%.4f)\n", radius);
}

void log_interpreter::cutter_radius_comp_start(Side direction)
{
...
void log_interpreter::cutter_radius_comp_stop()
{
	PRINT ("STOP_CUTTER_RADIUS_COMPENSATION()\n");
}

void log_interpreter::speed_feed_sync_start()
{
	PRINT ("START_SPEED_FEED_SYNCH()\n");
}

void log_interpreter::speed_feed_sync_stop()
{
...
void log_interpreter::arc(double end0, double end1, double axis0, double axis1, int rotation, double end_point, double a, double b, double c)
{
...
void log_interpreter::linear(const Position& pos)
{
...
void log_interpreter::probe(const Position& pos)
{
    double distance;
    double dx, dy, dz;
    double backoff;
...
void log_interpreter::dwell(double seconds)
{
	PRINT("DWELL(%.4f)\n", seconds);
}
...
void log_interpreter::spindle_start_clockwise()
{
...
void log_interpreter::spindle_start_counterclockwise()
{
...
void log_interpreter::spindle_speed(double r)
{
...
void log_interpreter::spindle_stop()
{
...
void log_interpreter::spindle_orient(double orientation, Direction direction)
{
...
void log_interpreter::tool_length_offset(double length)
{
	PRINT("USE_TOOL_LENGTH_OFFSET(%.4f)\n", length);
}

void log_interpreter::tool_change(int slot)
{
    PRINT("CHANGE_TOOL(%d)\n", slot);
    _active_slot = slot;
}
...
void log_interpreter::tool_select(int i)
{
	PRINT("SELECT_TOOL(%d)\n", i);
}
...
void log_interpreter::axis_clamp(Axis axis)
{
    PRINT("CLAMP_AXIS(%s)\n",
...
void log_interpreter::comment(const char *s)
{PRINT("COMMENT(\"%s\")\n", s);}

void log_interpreter::feed_override_disable()
{PRINT("DISABLE_FEED_OVERRIDE()\n");}

void log_interpreter::speed_override_disable()
{PRINT("DISABLE_SPEED_OVERRIDE()\n");}

void log_interpreter::feed_override_enable()
{PRINT("ENABLE_FEED_OVERRIDE()\n");}

void log_interpreter::speed_override_enable()
{PRINT("ENABLE_SPEED_OVERRIDE()\n");}

void log_interpreter::coolant_flood_off()
{
    PRINT("FLOOD_OFF()\n");
    _flood = 0;
}
...
void log_interpreter::coolant_flood_on()
{
    PRINT("FLOOD_ON()\n");
    _flood = 1;
}
...
void log_interpreter::message(const char *s)
{
	PRINT("MESSAGE(\"%s\")\n", s);
}

void log_interpreter::coolant_mist_off()
{
    PRINT("MIST_OFF()\n");
    _mist = 0;
}
...
void log_interpreter::coolant_mist_on()
{
    PRINT("MIST_ON()\n");
    _mist = 1;
}
...
void log_interpreter::pallet_shuttle()
{PRINT("PALLET_SHUTTLE()\n");}


void log_interpreter::probe_off()
{PRINT("TURN_PROBE_OFF()\n");}

void log_interpreter::probe_on()
{PRINT("TURN_PROBE_ON()\n");}

void log_interpreter::axis_unclamp(Axis axis)
{
...
void log_interpreter::program_stop()
{PRINT("PROGRAM_STOP()\n");}

void log_interpreter::program_optional_stop()
{PRINT("OPTIONAL_PROGRAM_STOP()\n");}

void log_interpreter::program_end()
{PRINT("PROGRAM_END()\n");}
...
double log_interpreter::feed_rate() const
{
    return _feed_rate;
}
...
bool log_interpreter::coolant_flood() const
{
    return _flood;
}
...
Units log_interpreter::units() const
{
...
bool log_interpreter::coolant_mist() const
{
    return _mist;
}
...
Motion log_interpreter::motion_mode() const
{
    return _motion_mode;
}


void log_interpreter::get_parameter_filename(char* filename, size_t max_size) const
{
...
Plane log_interpreter::plane() const
{
    return _active_plane;
}


Position log_interpreter::current_position() const
{
	return program_pos;
}

Position log_interpreter::probe_position() const
{
	return probe_pos;
}
...
double log_interpreter::probe_value() const
{
    return 1.0;
}
...
bool log_interpreter::queue_empty() const
{
    return 1;
}
...
double log_interpreter::spindle_speed() const
{
...
Direction log_interpreter::spindle_state() const
{
   ...
   tool in the spindle. */
int log_interpreter::tool_slot() const
{
    return _active_slot;
}
...
unsigned int log_interpreter::tool_max() const
{
    return _tool_max;
}
   ...
   /* Returns the CANON_TOOL_TABLE structure associated with the tool
      in the given pocket */
Tool log_interpreter::tool(int pocket) const
{
...
double log_interpreter::rapid_rate() const
{
    return _traverse_rate;
}

log_interpreter::log_interpreter()
{
	init();
}
